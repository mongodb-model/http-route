...
```bash
$ yarn add @mongodb-model/http-route 
...
$ npm i @mongodb-model/http-route
...
```bash
 const Base = require('@mongodb-model/http-route');
 const base = new Base();
 base.apiGet();
...
or 
```bash
 class MyWonderfulClass extends require('@mongodb-model/http-route') {
